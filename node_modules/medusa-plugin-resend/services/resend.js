...
class ResendService extends medusa_interfaces_1.NotificationService {
    /**
    ...
    constructor({ storeService, orderService, returnService, swapService, cartService, lineItemService, claimService, fulfillmentService, fulfillmentProviderService, totalsService, productVariantService, }, options) {
        ...
        this.templatePath_ = (this.options_.template_path.startsWith('/')) ?
            path_1.default.resolve(this.options_.template_path) : // The path given in options is absolute
            path_1.default.join(__dirname, '../../..', this.options_.template_path); // The path given in options is relative
        ...
        this.transporter_ = new resend_1.Resend(this.options_.api_key);
    }
    async sendNotification(event, eventData, attachmentGenerator) {
        let templateId = this.getTemplateId(event);
        if (!templateId) {
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, "Resend service: No template was set for this event");
        }
        const data = await this.fetchData(event, eventData, attachmentGenerator);
        if (!data) {
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, "Resend service: Invalid event data was received");
        }
        if (data.locale) {
        ...
        if (this.options_.subject_template_type === 'text') {
            sendOptions.subject = fs_1.default.existsSync(path_1.default.join(this.templatePath_, templateId, 'subject.txt')) ?
                fs_1.default.readFileSync(path_1.default.join(this.templatePath_, templateId, 'subject.txt'), "utf8") : null;
        }
        else {
        ...
        if (!sendOptions.subject || (!sendOptions.html && !sendOptions.text && !sendOptions.react)) {
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, "Resend service: The requested templates were not found. Check template path in config.");
        }
    ...
    async compileSubjectTemplate(templateId, data) {
        const subjectTemplate = fs_1.default.existsSync(path_1.default.join(this.templatePath_, templateId, 'subject.hbs')) ?
            handlebars_1.default.compile(fs_1.default.readFileSync(path_1.default.join(this.templatePath_, templateId, 'subject.hbs'), "utf8")) : null;
        if (subjectTemplate)
    ...
    async compileBodyTemplate(templateId, data) {
        const htmlTemplate = fs_1.default.existsSync(path_1.default.join(this.templatePath_, templateId, 'html.hbs')) ?
            handlebars_1.default.compile(fs_1.default.readFileSync(path_1.default.join(this.templatePath_, templateId, 'html.hbs'), "utf8")) : null;
        const textTemplate = fs_1.default.existsSync(path_1.default.join(this.templatePath_, templateId, 'text.hbs')) ?
            handlebars_1.default.compile(fs_1.default.readFileSync(path_1.default.join(this.templatePath_, templateId, 'text.hbs'), "utf8")) : null;
        return {
    ...
    async compileReactTemplate(templateId, data) {
        var _a;
        if (fs_1.default.existsSync(path_1.default.join(this.templatePath_, templateId, 'html.js'))) {
            let EmailTemplate = await (_a = path_1.default.join(this.templatePath_, templateId, 'html.js'), Promise.resolve().then(() => __importStar(require(_a))));
...
ResendService.identifier = "resend";
exports.default = ResendService;
